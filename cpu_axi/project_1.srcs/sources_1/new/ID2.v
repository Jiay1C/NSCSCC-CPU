...
module ID2(
    ...
    always @(*) begin
        ...
        6'b000000: //R
        begin 
            MemWrite<=0;MemtoReg<=0;SignExt<=0;PCSrc<=0;outPC<=RDataA;BranchType<=6'b0;MemSize<=2'h0;MemSignExt<=0;MTC0<=0;ERET<=0;CP0toReg<=0;MemEN<=0;
            case (funct)
        ...
        //I
        6'b001000:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //ADDI
        6'b001001:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=1;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //ADDIU
        6'b001010:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000000000100;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //SLTI
        6'b001011:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000000001000;SignExt<=1;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //SLTIU
        6'b001100:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000000010000;SignExt<=1;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //ANDI
        6'b001101:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000001000000;SignExt<=1;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //ORI
        6'b001110:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000010000000;SignExt<=1;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //XORI
        6'b001111:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b100000000000;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b00;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //LUI
        6'b100011:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=1;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //LW
        6'b100000:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=1;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h2;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //LB
        6'b100100:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=1;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h2;MemSignExt<=1;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //LBU
        6'b100001:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=1;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h1;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //LH
        6'b100101:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=0;RegWrite<=1;MemtoReg<=1;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h1;MemSignExt<=1;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //LHU
        6'b101011:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=1;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b11;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //SW
        6'b101000:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=1;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h2;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b11;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //SB
        6'b101001:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h1;MemWrite<=1;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h1;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b11;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=1; end //SH

        //J
        6'b000010:begin RegDst<=2'h2;ALUSrc1<=0;ALUSrc2<=2'h0;MemWrite<=0;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=1;outPC<={inPC[31:28],JumpTarget,2'b00};BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b00;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //J
        6'b000011:begin RegDst<=2'h2;ALUSrc1<=0;ALUSrc2<=2'h0;MemWrite<=0;RegWrite<=1;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=1;outPC<={inPC[31:28],JumpTarget,2'b00};BranchType<=6'b0;ExtratoReg<=1;ExtraDataSrc<=3'b001;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b00;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //JAL

        6'b000100:begin RegDst<=2'h2;ALUSrc1<=0;ALUSrc2<=2'h0;MemWrite<=0;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000010;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=1;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b000001;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b11;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //BEQ
        6'b000101:begin RegDst<=2'h2;ALUSrc1<=0;ALUSrc2<=2'h0;MemWrite<=0;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000010;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=1;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b000010;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b11;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //BNE
        6'b000111:begin RegDst<=2'h2;ALUSrc1<=0;ALUSrc2<=2'h2;MemWrite<=0;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000010;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=1;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b001000;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //BGTZ
        6'b000110:begin RegDst<=2'h2;ALUSrc1<=0;ALUSrc2<=2'h2;MemWrite<=0;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000010;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=1;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b010000;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end //BLEZ
        6'b000001:begin RegDst<=2'h2;ALUSrc1<=0;ALUSrc2<=2'h2;MemWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000010;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=1;PCSrcForward<=0;outPC<=32'b0;ExtratoReg<=1;ExtraDataSrc<=3'b001;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b01;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0;
                        case (rt)
        ...
        6'b010000:begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h0;MemWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000001;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;MemEN<=0;
                    case (rs)
        ...
        default: begin RegDst<=2'h0;ALUSrc1<=0;ALUSrc2<=2'h0;MemWrite<=0;RegWrite<=0;MemtoReg<=0;ALUOP<=12'b000000000000;SignExt<=0;HIWrite<=0;LOWrite<=0;PCSrc<=0;PCSrcForward<=0;outPC<=32'b0;BranchType<=6'b0;ExtratoReg<=0;ExtraDataSrc<=3'b000;MemSize<=2'h0;MemSignExt<=0;HILOWrite<=0;MDUOP<=4'b0000;RDataValid<=2'b00;MTC0<=0;ERET<=0;ExcCode<=5'b11111;CP0toReg<=0;MemEN<=0; end
        endcase
    end
