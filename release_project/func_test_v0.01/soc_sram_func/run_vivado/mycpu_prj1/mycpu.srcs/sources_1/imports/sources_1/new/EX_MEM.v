...
module EX_MEM(
    ...
    output reg [1:0]outOPType
    );

    always @(posedge CLK) begin
        if(RST) {outPC,outALURes,outCF,outOF,outSF,outZF,outMemWData,outWAddr,outRegWrite,outHIWrite,outLOWrite,outHILOWrite,outPCSrc,outMemWrite,outMemtoReg,outExtratoReg,outBranchType,outExtraData,outMemSize,outMemSignExt,outHILO,outMTC0,outERET,outExcCode,outCP0toReg,outCP0Addr,outPC0,outMemEN,outException,outOPType}<=0;
        else begin
            outPC<=inPC;
            outALURes<=inALURes;
            outCF<=inCF;
