...
module CU(
    ...
    assign JumpPause=JumpPause0;
    //assign BranchPause=BranchPause0||BranchPause1;
    //assign BranchPause=BranchPause0;
    assign BranchPause=BranchPause0||BranchPause1;
    //assign ERETPause=ERETPause0||ERETPause1;
    assign ERETPause=ERETPause0;
    //assign ControlPause=JumpPause||BranchPause||ERETPause;
    assign ControlPause=BranchPause||ERETPause;
    //assign PCEN=~BranchPause0;
    assign PCEN=~BranchPause;
    //assign ControlPause=0;
    //assign PCEN=1;
    assign outMDUPause=MDUPause1||MDUPause;
    ...
    always @(*) begin
        ...
        //else if(IMPause||DMPause)      begin IF_EN=0;IF_ID_EN=0;ID_EN=0;ID_EX_EN=0;EX_EN=0;EX_MEM_EN=0;MEM_EN=0;MEM_WB_EN=0;WB_EN=0; end
        else              begin IF_EN=1;IF_ID_EN=1;ID_EN=1;ID_EX_EN=1;EX_EN=1;EX_MEM_EN=1;MEM_EN=1;MEM_WB_EN=1;WB_EN=1; end
    end

    always @(posedge CLK) begin
        if(RST) {WRPause0,JumpPause1,JumpPause0,BranchPause1,BranchPause0,ERETPause0,ERETPause1,MDUPause0}<=0;
        else begin
            WRPause0<=WRPause_WRConflict;
            JumpPause0<=Jump_ID;
            JumpPause1<=JumpPause0;
            //BranchPause0<=Branch_ID||Branch_EX||Branch_MEM;
            BranchPause0<=Branch_EX||Branch_MEM;
            BranchPause1<=BranchPause0;
            ERETPause0<=ERET_EX||ERET_MEM||ERET_WB;
    ...
    always @(negedge CLK) begin
        if(RST) {WRPause,MDUPause1}<=0;
        else begin WRPause<=WRPause0;MDUPause1<=MDUPause; end
    end
